...
4. Decide if a variable is visible (scope) in a nested block. :evaluate:
...
2. Evaluate the execution of a loop control structure. :evaluate:
    1. List the order of operations for both `for` and `while` loop control
       structures in Java/Processing.
3. Discover the cause of crashing software by inspecting a stack trace.
   :program:
